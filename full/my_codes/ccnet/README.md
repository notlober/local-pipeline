...
[![CircleCI](https://circleci.com/gh/facebookresearch/ccnet.cc_net.svg?style=svg)](https://circleci.com/gh/facebookresearch/ccnet.cc_net)
...
You can launch the full pipeline using `python -m ccnet.cc_net`.

* `python -m ccnet.cc_net --help` shows help
* `python -m ccnet.cc_net --dump 2019-13` treats a specific snapshot
* `python -m ccnet.cc_net -l my -l gu` 
restricts to specific languages
* `python -m ccnet.cc_net --lm_dir my_lms/` uses custom LMs
* `python -m ccnet.cc_net --lang_threshold 0.3` set a specific field in `mine.Config`
* `python -m ccnet.cc_net --config test` runs on a tiny subset of a snapshot
* `python -m ccnet.cc_net --config config/my_config.json` uses configuration from the given config file
...
```sh
python -m ccnet.cc_net --conf reproduce --dump 2019-09
```

## Extract XLM-R data

[Unsupervised Cross-lingual Representation Learning at Scale (XLM-RoBERTa)](https://arxiv.org/pdf/1911.02116.pdf)
paper was trained on data extracted by an internal version of ccnet.cc_net.

Due to the format being a little bit different please use the following command instead:

```sh
python ccnet.cc_net/tools/dl_cc_100.py --help
python ccnet.cc_net/tools/dl_cc_100.py --outdir data_cc100 --process 8
```
...
By contributing to `ccnet.cc_net`, you agree that your contributions will be licensed
